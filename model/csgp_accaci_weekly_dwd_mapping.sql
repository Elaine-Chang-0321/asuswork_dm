{{ config(
    ...
    meta= {
        'job': "csgp_accaci_weekly_dwd_mapping",
        'alias': "acc aci data with various mappings",
    },
    location = 's3://consumer-npspo/Tables/ads_dev/csgp_accaci_weekly_dwd_mapping'    
)}}
With accaci_raw_maxds AS (
    SELECT * 
    FROM {{ source('ods', 's_apza002bid_csgp_accaci_local_data_weekly') }}
    WHERE ds = (SELECT MAX(ds) FROM {{ source('ods', 's_apza002bid_csgp_accaci_local_data_weekly') }})
),
max_period_data AS (
...
mapping_1179 AS (
    ...
    /* Mapping Costgap 計算 */
    , mc.opp_rebate * raw.all_qty AS opp_amount
    , mc.cpu_extra_rebate * raw.all_qty AS extra_rebate
    , mc.nv_rebate * raw.all_qty AS nv_sellout_rebate
    ...
    -- , data_1179_pn.ship_fee_hq AS ship_fee_hq_pn
    -- , data_1179_pn_max.ship_fee_hq AS ship_fee_hq_pn_max
    -- , data_1179_model_cpu.ship_fee_hq AS ship_fee_hq_model_cpu
    -- , data_1179_model_cpu_max.ship_fee_hq AS ship_fee_hq_model_cpu_max
    -- , CASE 
    --     WHEN raw.source = 'ACCRAW' AND raw.cost_period != '由新到舊' THEN 
    --         COALESCE(
    --             data_1179_pn.ship_fee_hq, 
    --             data_1179_pn_max.ship_fee_hq, 
    --             data_1179_model_cpu.ship_fee_hq, 
    --             data_1179_model_cpu_max.ship_fee_hq
    --         )
    --     WHEN (raw.source = 'ACCRAW' AND raw.cost_period = '由新到舊') OR raw.source = 'ACIRAW' THEN 
    --         COALESCE(
    --             data_1179_pn_max.ship_fee_hq, 
    --             data_1179_model_cpu_max.ship_fee_hq
    --         )
    ...
    , CAST(os_incentive.inc AS decimal(34, 15)) * raw.all_qty AS os_incentive
    , CASE WHEN raw.source = 'ACCRAW' THEN  CAST(csc.reserve_margin AS decimal(34, 15)) ELSE CAST(raw.csc_ratio AS decimal(34, 15)) END AS csc_ratio_processed
    , CASE WHEN raw.source = 'ACCRAW' THEN  (CAST(raw.revenue_rmb AS decimal(34, 15)) * CAST(csc.reserve_margin AS decimal(34, 15)))/CAST(fx.fx_rate AS decimal(34, 15)) ELSE CAST(raw.csc_amt AS decimal(34, 15)) END AS csc_amt_processed
    , CASE WHEN raw.source = 'ACCRAW' THEN  (CAST(raw.revenue_rmb AS decimal(34, 15)) * CAST(csc.reserve_margin AS decimal(34, 15)))/CAST(fx.fx_rate_ex AS decimal(34, 15)) ELSE CAST(raw.csc_amt AS decimal(34, 15)) END AS csc_amt_fxdiff
    , CASE WHEN raw.source = 'ACCRAW' THEN  (CAST(raw.revenue_rmb AS decimal(34, 15)) * CAST(csc.reserve_margin AS decimal(34, 15))) ELSE NULL END AS csc_amt_rmb
    , mk.cte AS is_cte
    FROM accaci_raw raw
    /* --- Mapping Joins --- */
    ...
    LEFT JOIN csc_data_raw csc
        ...
        AND raw.warranty = CAST(csc.warranty AS int)
