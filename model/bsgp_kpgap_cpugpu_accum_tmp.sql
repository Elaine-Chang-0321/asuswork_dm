CREATE EXTERNAL TABLE `bsgp_kpgap_cpugpu_accum_tmp`(
    `product_line` string COMMENT 'from deserializer', 
    `category` string COMMENT 'from deserializer', 
    `item_no` string COMMENT 'from deserializer', 
    ...
    `pac` string COMMENT 'from deserializer',
    `rebate` string COMMENT 'from deserializer'
  )
...
LOCATION 's3://consumer-npspo/StaticFiles/armourycrate/bsgp_kpgap_cpugpu_accum_tmp/'
TBLPROPERTIES (
  'classification' = 'csv',
