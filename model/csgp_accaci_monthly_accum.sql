{{ config(
    ...
    meta= {
        'job': "csgp_accaci_monthly_accum",
        'alias': "acc aci data with various mappings",
    },
    location = 's3://consumer-npspo/Tables/ads_dev/csgp_accaci_monthly_accum'    
)}}
-- 从 ads_dev.csgp_accaci_monthly_dwd_mapping 读取数据并进行多层计算
...
calc_1_base_gp_calculation AS (
    ...
    FROM {{ source('ads_dev', 'csgp_accaci_monthly_dwd_mapping') }}
    WHERE ds = (SELECT MAX(ds) FROM {{ source('ads_dev', 'csgp_accaci_monthly_dwd_mapping') }})
),
