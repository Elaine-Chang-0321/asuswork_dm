...
def analyze_and_insert(model_id, file_name, file_path, system_prompt, model_version, db_conn):
    ...
    try:
        ...
        if not isinstance(data, list):
            print(f"[ERROR] 檔案 {file_name} 預期 LLM 回傳 List，跳過寫入。")
            return
            
        if len(data) == 0:
            print(f"  - 檔案 {file_name} 似乎沒有產出任何欄位。")
            return
        ...
        for col_info in data:
            ...
            column_type = col_info.get("column_type", "").strip()
            source_table = col_info.get("source_table", "").strip()
            sql_expression = col_info.get("sql_expression", "").strip()
            source_sql_fragment = col_info.get("source_sql_fragment", "").strip()
            
            if not table_name or not column_name:
                continue
                
            print(f"  - 找到欄位: {table_name}.{column_name}")
            
            # 從資料庫查詢是否已有此 table_name + column_name，若無則取最大 column_id + 1
            column_id = None
            try:
                cursor = db_conn.cursor()
                cursor.execute("SELECT column_id FROM dataserviceflow_dm.columnlist WHERE table_name = %s AND column_name = %s", (table_name, column_name))
                row = cursor.fetchone()
                if row:
                    column_id = row[0]
                else:
                    cursor.execute("SELECT MAX(CAST(column_id AS UNSIGNED)) FROM dataserviceflow_dm.columnlist")
                    max_row = cursor.fetchone()
                    max_id = max_row[0] if max_row and max_row[0] is not None else 0
                    column_id = f"{(max_id + 1):06d}"
                cursor.close()
            except Exception as e:
                print(f"    [ERROR] 查詢 column_id 失敗: {e}")
                continue
            ...
            try:
                cursor = db_conn.cursor()
                insert_query = """
                    INSERT INTO dataserviceflow_dm.columnlist 
                    (column_id, column_name, table_name, model_name, column_type, source_table, sql_expression, source_sql_fragment, dw_ins_time)
                    ...
                    source_sql_fragment=VALUES(source_sql_fragment),
                ...
                db_conn.commit()
                cursor.close()
                print(f"    [SUCCESS] {table_name}.{column_name} 已存入 (column_id: {column_id})")
                
            except Exception as db_err:
                print(f"    [ERROR] 資料庫寫入失敗 ({column_name}): {db_err}")
...
def main():
    ...
    active_prompt_section = "Prompt_ColumnList"
