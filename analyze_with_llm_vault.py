...
def main():
    ...
    # ★ 關鍵：在呼叫 aocc_client_vault 之前，我們必須先把 .env 載入到系統變數裡
    ...
    print("[INFO] 正在請求 LLM 模型分析程式碼，此過程透過 aocc_client_vault 進行連線，請稍候...")
    
    # 從 .env 中讀取我們想要使用的模型
    model_version = os.getenv("AOCC_MODEL_VERSION", "gpt41")
    
    # 4. 呼叫 aocc_client_vault 進行分析
    try:
        result = aocc_client_vault.ask_llm(
            model=model_version,
        ...
        print(f"[FATAL] 透過 aocc_client_vault 呼叫 LLM 失敗: {e}")
        sys.exit(1)
